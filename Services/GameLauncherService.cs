...
public static class GameLauncherService
{
    ...
    public static void LaunchDosPrompt(GameConfiguration gameConfig, string? dosboxExePath, IWin32Window owner)
    {
        if (string.IsNullOrEmpty(dosboxExePath) || !File.Exists(dosboxExePath))
        {
            MessageBox.Show(owner, $"DOSBox executable path is not configured or the configured path is invalid. Please check the settings.", "Launch Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        // Mount Floppy Disks
        if (gameConfig.DiscImages.Any())
        {
            var validDiscPaths = gameConfig.DiscImages
                .Where(discInfo => File.Exists(discInfo.FilePath))
                .Select(discInfo => $"'{discInfo.FilePath}'");

            if (validDiscPaths.Any())
            {
                dosboxArgs.Add($"-c \"IMGMOUNT A {string.Join(" ", validDiscPaths)} -t floppy\"");
            }
        }
        ...
        {
            var validIsoPaths = gameConfig.IsoImages
                .Where(isoInfo => File.Exists(isoInfo.FilePath))
                .Select(isoInfo => $"'{isoInfo.FilePath}'");

            if (validIsoPaths.Any())
            {
                dosboxArgs.Add($"-c \"IMGMOUNT D {string.Join(" ", validIsoPaths)} -t iso\"");
            }
        }
        ...
        string arguments = string.Join(" ", dosboxArgs);
        AppLogger.Log($"Launching DOSBox for prompt with command: {dosboxExePath} {arguments}");
